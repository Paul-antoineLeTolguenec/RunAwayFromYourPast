...
def parse_args():
    ...
    parser.add_argument("--plot-density", type=bool, default=False)
    ...
    # ducb
    parser.add_argument("--gamma-ucb", type=float, default=0.90)
    parser.add_argument("--cut-off-treshold", type=int, default=20)
    parser.add_argument("--n-reconf", type=int, default=10)
...
class Agent(nn.Module):
    def __init__(self, envs, n_agent, n_reconf):
        super().__init__()
        self.critic = nn.Sequential(
            layer_init(nn.Linear(np.array(envs.single_observation_space.shape).prod()+n_reconf, 64)),
            nn.Tanh(),
...
def cut_arm(ducb, z, cut_off_treshold, epoch_reconf, ve ) :
    idx_arms_to_cut = []
    counts = ducb.counts
    first = np.argmax(counts)
    for idx_arm in range(len(counts)):
        if counts[first] >= cut_off_treshold + counts[idx_arm]:
            idx_arms_to_cut.append(idx_arm)
    if len(idx_arms_to_cut) > 0:
        # fix the arm
        ve.z[np.array(idx_arms_to_cut), epoch_reconf] = first + 1
        # z 
        z = ve.shuffle(ve.z, epoch=epoch_reconf).clone()
        # cut the arm
        epoch_reconf += 1
        # reset the counts
        ducb.counts = np.zeros_like(counts)
        # reset the values
        ducb.values = np.zeros_like(counts)
        # total plays
        ducb.total_plays = 1
    return z, epoch_reconf, ducb
...
if __name__ == "__main__":
    ...
    if args.make_gif:
        ...
        if not os.path.exists('gif'):
            os.makedirs('gif')
        writer_gif = imageio.get_writer('gif/v4_ppo_clean.mp4', fps=2)
    ...
    # Agent
    agent = Agent(envs, args.n_agent, args.n_reconf).to(device)
    ...
    last_mean_per_agent = torch.zeros(args.n_agent).to(device)
    # vector encoding
    ve = VE(n = args.n_agent, device = device, prob = torch.ones(args.n_agent)/args.n_agent, n_reconf = args.n_reconf)
    # ALGO Logic: Storage setup
    obs = torch.zeros((args.num_steps, args.num_envs) + envs.single_observation_space.shape).to(device)
    zs = torch.zeros((args.num_steps, args.num_envs) + (args.n_reconf,), dtype = torch.int64).to(device)
    ...
    # Discounted UCB
    ducb = DiscountedUCB(n_arms = args.n_agent, gamma = args.gamma_ucb)
    epoch_reconf = 0
    # full replay buffer
    obs_un =None
    z_un = None
    ...
    for update in range(1, num_updates + 1):
        # CUT ARM 
        z, epoch_reconf, ducb = cut_arm(ducb, z, args.cut_off_treshold, epoch_reconf, ve)
         # UCB Choice 
        for env_idx in range(args.num_envs):
            # select the arm
            idx_arm = ducb.select_arm()
            # update z 
            z[env_idx, epoch_reconf] = idx_arm + 1
        ...
        for step in range(0, args.num_steps):
            ...
            zs[step] = z
            z = ve.shuffle(z, epoch=epoch_reconf).clone()
        ...
        b_batch_z_rho_n = zs.permute(1,0,2).reshape(args.num_envs*args.num_rollouts, max_steps, args.n_reconf)
        ...
        if obs_un_train is not None and obs_un_train.shape[0] >= args.classifier_memory :  
            ...
            for epoch_classifier in range(classifier_epochs):
                ...
                mb_rho_n_z = b_batch_z_rho_n[idx_ep_rho, idx_step_rho, epoch_reconf].unsqueeze(-1)
        ...
        with torch.no_grad():
            ...
            # log_p_s_z = (log_p_s_z_nn - torch.mean(log_p_s_z_nn, dim=0).unsqueeze(0))/(torch.std(log_p_s_z_nn, dim=0).unsqueeze(0) + 1e-1)
            log_p_s_z = log_p_s_z_nn 
            # rewards
            rewards = log_p_s_z if update < args.start_explore else log_p_rho_un + args.lamda_im*log_p_s_z
            # mask entropy
            mask_entropy = (args.treshold_entropy <= log_p_rho_un_nn).float()
            # success
            mean_per_agent = log_p_rho_un_nn.mean(dim=0)
            success_per_agent =  (mean_per_agent >= args.treshold_success).float()
            
        ########################### UPDATE THE BUFFER ############################
        obs_permute = obs.permute(1,0,2)
        z_permute = zs.permute(1,0,2)
        # obs_un add 
        for idx in range(args.n_agent):
            z_idx = z[idx, epoch_reconf].item() - 1
            ...
            z_un = z_permute[z_idx].reshape(-1, args.n_reconf).clone() if z_un is None else (torch.cat([z_un, z_permute[z_idx].reshape(-1, args.n_reconf).clone()], dim=0) if (success_per_agent[z_idx] or update <  args.start_explore) else z_un)
        ...
        ########################### UPDATE DISCOUNTED UCB ############################
        for i in range(args.n_agent):
            z_idx = z[i, epoch_reconf].item() - 1
            ducb.update(chosen_arm=z_idx, reward=success_per_agent[i].item() * 0.05)
        ...
        b_zs = zs.reshape((-1,) + (args.n_reconf,))
        ...
        print('max log_p_s_z',torch.max(log_p_s_z, dim = 0).values)
        print('min log_p_s_z',torch.min(log_p_s_z, dim = 0).values)
        print('mean log_p_s_z',torch.mean(log_p_s_z_nn, dim = 0))
        print('max log_p_rho_un',torch.max(log_p_rho_un, dim = 0).values)
        print('min log_p_rho_un',torch.min(log_p_rho_un, dim = 0).values)
        print('mean log_p_rho_un',torch.mean(log_p_rho_un_nn, dim = 0))
        # print('std log_p_rho_un',torch.std(log_p_rho_un_nn))
        # print('sum mask_rewards dim 0',mask_entropy.sum(dim=0))
        print('update : ', update)
        print('success per agent', success_per_agent)
        print('count per arm', ducb.counts)
        print('value per arm', ducb.values)
        print('epoch reconf', epoch_reconf)
        ...
        if update % args.fig_frequency == 0 and args.make_gif and global_step > 0:
            with torch.no_grad():
                if args.plot_density : 
                    ...
                    obs_permute = obs.permute(1,0,2)
                    # cat 
                    data_to_plot = torch.cat([data_to_plot, obs_permute.reshape(-1, *envs.single_observation_space.shape)], dim = 0)
                    # # Plotting measure 
                    ...
                    # plot obs_un_train 
                    env_plot.ax.scatter(obs_un_train.cpu().numpy()[:,0], obs_un_train.cpu().numpy()[:,1], c = 'b', s=1)
                else : 
                    iter_plot  = obs_un.shape[0]
                    z_un_plot = z_un[last_iter: iter_plot].cpu().numpy() - 1
                    data_to_plot  = obs_un[last_iter: iter_plot].cpu().numpy()
                    for idx in range(args.n_agent):
                        mask_z = z_un_plot[:,epoch_reconf] == idx
                        env_plot.ax.scatter(data_to_plot[mask_z,0], data_to_plot[mask_z,1], c = colors[idx], s=1)
                    last_iter = iter_plot
