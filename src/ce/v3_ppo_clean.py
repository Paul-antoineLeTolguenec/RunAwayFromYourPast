...
if __name__ == "__main__":
    ...
    if args.make_gif:
        ...
        if not os.path.exists('gif'):
            os.makedirs('gif')
        writer_gif = imageio.get_writer('gif/v3_ppo_clean.mp4', fps=2)
    ...
    for update in range(1, num_updates + 1):
        ...
        for idx in range(args.n_agent):
            ...
            z_un = z_permute[z_idx].reshape(-1, 1).clone() if z_un is None else (torch.cat([z_un, z_permute[z_idx].reshape(-1, 1).clone()], dim=0) if (success_per_agent[z_idx] or update <  args.start_explore) else z_un)
        ...
        for i in range(args.n_agent):
            z_idx = z[i].item() - 1
            ducb.update(chosen_arm=z_idx, reward=success_per_agent[i].item() * 0.01)
        ...
        if update % args.fig_frequency == 0 and args.make_gif and global_step > 0:
            with torch.no_grad():
                if args.plot_density : 
                    ...
                    for idx in range(args.n_agent):
                        mask_z = z_un_plot == idx
                        mask_z = mask_z.squeeze(-1)
