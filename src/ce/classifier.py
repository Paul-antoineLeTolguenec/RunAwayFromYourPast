...
class Classifier(torch.nn.Module):
    def __init__(self, observation_space,device, env_max_steps, 
        ...
        if feature_extractor:
            ...
            self.fc1 = spectral_norm(torch.nn.Linear(observation_space.shape[0], 128).to(device), n_power_iterations =iter_lip)
            self.fc2 = spectral_norm(torch.nn.Linear(128, 64).to(device), n_power_iterations =iter_lip)
            self.fc3 = spectral_norm(torch.nn.Linear(64, 1).to(device), n_power_iterations =iter_lip)
        else:
