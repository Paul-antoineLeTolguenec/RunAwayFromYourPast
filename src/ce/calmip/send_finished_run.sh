        ...
        if [ -f "$METADATA_FILE" ]; then
            ...
            if contains_algo "$METADATA_FILE"; then
                ...
                if [ "$RUN_STATUS" == "finished" ]; then
                    ...
                    RUN_NAME=$(extract_arg_value "$METADATA_FILE" "run_name") # Assurez-vous que "run_name" correspond à l'argument dans le fichier JSON
                    echo "Le run $RUN_NAME n'est pas terminé (status: $RUN_STATUS)."
                fi
            fi
        fi
    fi
