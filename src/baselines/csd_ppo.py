...
class Discriminator(torch.nn.Module):
    ...
    def lipshitz_loss(self, s, s_next, z, d):
        ...
        lsd = -(( (phi_s_next - phi_s) * z).sum(dim = -1)).mean()
        csd_regu = -(self.lambda_csd.detach()*torch.min(self.eps*torch.ones_like(d), self.d_csd(s,s_next) - ((phi_s_next - phi_s)* z).sum(dim=1))).mean()
        return lsd + csd_regu
...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        batch_actions = actions.permute(1,0,2).reshape(-1, actions.shape[-1]).to(device)
        batch_dones = dones.permute(1,0,2).reshape(-1).to(device)
        if obs_un is not None:
            for _ in range(args.discriminator_epochs):
                ...
                mb_rho_actions = batch_actions[idx_rho]
                mb_rho_next_obs = batch_obs[idx_rho+1]
                mb_rho_z = batch_zs[idx_rho]
                mb_rho_d = batch_dones[idx_rho].unsqueeze(-1)
                # un
                idx_un = np.random.randint(0, obs_un.shape[0], args.discriminator_batch_size)
                mb_un_obs = torch.tensor(obs_un[idx_un]).to(device)
                mb_un_actions = torch.tensor(actions_un[idx_un]).to(device)
                mb_un_next_obs = torch.tensor(next_obs_un[idx_un]).to(device)
                mb_un_z = torch.tensor(z_un[idx_un]).to(device)
                mb_un_d = torch.tensor(dones_un[idx_un]).unsqueeze(-1).to(device)

                beta = args.beta_ratio
                # MLH loss
                mlh_loss = beta*discriminator.mlh_loss(mb_rho_obs, mb_rho_next_obs, mb_rho_d) + (1-beta)*discriminator.mlh_loss(mb_un_obs, mb_un_next_obs, mb_un_d)
                optimizer_discriminator.zero_grad()
                mlh_loss.backward()
                optimizer_discriminator.step()

                # LIPSHITZ LOSS
                loss = beta*discriminator.lipshitz_loss(mb_rho_obs, mb_rho_next_obs, mb_rho_z, mb_rho_d) + (1-beta)*discriminator.lipshitz_loss(mb_un_obs, mb_un_next_obs, mb_un_z, mb_un_d)
                optimizer_discriminator.zero_grad()
                loss.backward()
                optimizer_discriminator.step()

                # LAMBDA LOSS
                loss_lambda = beta*discriminator.lambda_loss(mb_rho_obs, mb_rho_next_obs, mb_rho_z, mb_rho_d) + (1-beta)*discriminator.lambda_loss(mb_un_obs, mb_un_next_obs, mb_un_z, mb_un_d)
                optimizer_discriminator.zero_grad()
