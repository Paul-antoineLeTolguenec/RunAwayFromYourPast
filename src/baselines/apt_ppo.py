...
class Encoder(torch.nn.Module):
    ...
    def get_knn_sum(self, s, states, k):
        """
        Compute the sum of distances for the k nearest neighbors of 's' relative to a batch 'states'.
        :param s: The reference state (a tensor of size [1, input_size])
        :param states: The batch of states (a tensor of size [N, input_size])
        :param k: The number of nearest neighbors to consider
        :return: The sum of distances of the k nearest neighbors
        """
        # Encode the states
        s_encoded = self.forward(s)
        states_encoded = self.forward(states)

        # Calculate Euclidean distances from 's' to all states in the batch
        distances = torch.norm(s_encoded - states_encoded, dim=1)

        # Get the k smallest distances
        k_smallest_distances, _ = torch.topk(distances, k, largest=False)

        # Compute the sum of these k smallest distances
        distance_sum = k_smallest_distances.sum()
    ...
    for update in range(1, num_updates + 1):
        ...
        if obs_un is not None:
            for _ in range(args.encoder_epochs):
                ...
                loss = encoder.contrastive_loss(sk, sv)
                loss.backward()
