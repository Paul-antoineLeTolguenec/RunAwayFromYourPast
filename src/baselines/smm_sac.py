...
class Args:
    ...
    nb_epoch_before_training_vae: int = 8
    """ nb epoch between each training """
    ...
    lambda_vae: float = 100.0
    """the lambda of the VAE"""
...
"""
    ...
    classifier_diayn = Classifier(observation_space=envs.single_observation_space, nb_skill=args.nb_skill, device=device, env_id=args.env_id, feature_extractor = True)
    ...
    for global_step in range(args.total_timesteps):
        ...
        # VAE TRAINING
        if global_step % args.nb_epoch_before_training_vae*max_step == 0 and global_step > args.learning_starts:
            mean_vae_loss = 0
        ...
        if global_step > args.learning_starts:
            with torch.no_grad():
                ...
                diayn_reward = (torch.log(torch.sum(classifier_diayn(b_obs)*b_z, dim = -1) + 1e-3) - torch.log(torch.tensor(1/args.nb_skill))) * args.lambda_diayn 
                aux_reward = vae.loss(b_obs, reduce=False) * args.lambda_vae
                intrinsic_reward = diayn_reward + aux_reward
            ...
            if global_step % 100 == 0:
                ...
                "specific/diayn_reward_mean" : diayn_reward.mean().item(),
                "specific/diayn_reward_max" : diayn_reward.max().item(),
                "specific/diayn_reward_min" : diayn_reward.min().item(),
                "specific/aux_reward_mean" : aux_reward.mean().item(),
                "specific/aux_reward_max" : aux_reward.max().item(),
                "specific/aux_reward_min" : aux_reward.min().item(),
                }, step = global_step) if args.track else None
