...
class ICM(nn.Module):   
    ...
    def loss(self, batch, reduce = True):
        ...
        # feature encoding
        phi = self.feature(batch.observations)
        next_phi = self.feature(batch.next_observations)
        # inverse model
        pred_a = self.inverse(phi, next_phi)
        # forward model
        pred_phi = self.forward(phi, batch.actions)
        # losses
        # inverse_loss : MSE for continuous action space
        inverse_loss = F.mse_loss(pred_a, batch.actions)
...
if __name__ == "__main__":
    ...
    icm_net = ICM(envs.single_observation_space.shape[0], envs.single_action_space.shape[0], device=device, beta=args.beta)
    q_optimizer = optim.Adam(list(qf1.parameters()) + list(qf2.parameters()), lr=args.q_lr)
    actor_optimizer = optim.Adam(list(actor.parameters()), lr=args.policy_lr)
    icm_optimizer = optim.Adam(list(icm_net.parameters()), lr=args.icm_lr)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            if global_step % args.q_frequency == 0:
                for _ in range(args.q_frequency):
                    ...
                    with torch.no_grad():
                        ...
                        rewards = icm_net.loss(data, reduce=False).detach().flatten()
                        # normalize rewards
            ...
            if global_step % args.icm_frequency == 0:
                icm_loss, inverse_loss, forward_loss = icm_net.loss(data, reduce=True)
            ...
            if global_step % 100 == 0:
                ...
                writer.add_scalar("losses/inverse_loss", inverse_loss.item(), global_step)
        ...
        if global_step % args.fig_frequency == 0 and args.do_fig and global_step > 0:
            # Plotting measure 
            with torch.no_grad() : 
                m_n = icm_net.loss(rb.full_rb_batch(), reduce=False).flatten().cpu().numpy()
