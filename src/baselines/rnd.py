...
def parse_args():
    ...
    parser.add_argument("--rnd-lr", type=float, default=1e-3)
    parser.add_argument("--rnd-frequency", type=int, default=256)
...
class RND(nn.Module):   
    def __init__(self, state_dim, device):
        ...
        self.f2_t = nn.Linear(256, 64, device=device)
...
if __name__ == "__main__":
    ...
    rnd_net = RND(envs.single_observation_space.shape[0], device)
    q_optimizer = optim.Adam(list(qf1.parameters()) + list(qf2.parameters()), lr=args.q_lr)
    actor_optimizer = optim.Adam(list(actor.parameters()), lr=args.policy_lr)
    rnd_optimizer = optim.Adam(list(rnd_net.parameters()), lr=args.rnd_lr)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if global_step > args.learning_starts:
            ...
            if global_step % args.q_frequency == 0:
                for _ in range(args.q_frequency):
                    ...
                    with torch.no_grad():
                        ...
                        rewards = rnd_net.loss(data.observations, reduce=False).detach().flatten()
                        # rewards
            ...
            if global_step % args.rnd_frequency == 0:
                rnd_loss = rnd_net.loss(data.observations)
                # print('rnd_loss',rnd_loss)
        ...
        if global_step % args.fig_frequency == 0 and args.do_fig and global_step > 0:
            # Plotting measure 
            with torch.no_grad() : 
                m_n = rnd_net(torch.Tensor(rb.observations[:rb.pos]).to(device)).detach().cpu().numpy()
