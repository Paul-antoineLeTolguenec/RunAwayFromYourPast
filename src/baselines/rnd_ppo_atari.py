...
class RND(nn.Module):   
    def __init__(self, state_dim, device):
        ...
        self.fc1 = torch.nn.Linear(512, 128).to(device)
        self.fc2 = torch.nn.Linear(128, 1).to(device)
        # target network
        ...
        self.fc1_t = torch.nn.Linear(512, 128).to(device)
        self.fc2_t = torch.nn.Linear(128, 1).to(device)
...
if __name__ == "__main__":
    ...
    rnd = RND(state_dim=envs.single_observation_space.shape[0], 
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        ######################################*** MODULE TRAINING ***######################################
        b_batch_obs = obs.reshape(-1, *envs.single_observation_space.shape)
        for rnd_update in range(args.rnd_epochs):
            idx_mb_obs = torch.randint(0, b_batch_obs.shape[0], (args.minibatch_size,))
            idx_mb_next_obs = idx_mb_obs + 1
            mb_obs = b_batch_obs[idx_mb_obs]
            rnd_loss = rnd.loss( mb_obs, reduce=True)
        ...
        with torch.no_grad():
            # update the reward
            rewards_intrinsic = rnd.loss(obs.reshape(-1, *envs.single_observation_space.shape), 
                                reduce=False).unsqueeze(-1)
            rewards_intrinsic = rewards_intrinsic.reshape(args.num_steps, args.num_envs)
