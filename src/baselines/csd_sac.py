...
class Args:
    ...
    lr_discriminator_CSD: float = 1e-4
    """the learning rate of the discriminator"""
    lambda_reward_CSD: float = 1.0
    """weight for the reward maximizing the wasserstein equivalent of the Mutual Information"""
    epsilon_CSD: float = 1e-3
    """relaxing constant"""
    lr_lambda_CSD: float = 1e-1
    """ lambda CSD learning rate """
    lambda_CSD_init: float = 30.0 
    ...
    lip_cte_CSD: float = 1.0
    """ the constant of the lipschitz for CSD """
...
class Discriminator_CSD(torch.nn.Module):
    def __init__(self,  
                state_dim, 
                z_dim,
                env_name, 
                ...
                scale_l = 1.0,
                lip_cte = 1.0):
        super(Discriminator_CSD, self).__init__()
        ...
        self.lip_cte = lip_cte
         # prediction model 
        self.pred_l1 = torch.nn.Linear(state_dim, 256) 
        self.pred_l2 = torch.nn.Linear(256, 64)
        self.pred_l3 = torch.nn.Linear(64, state_dim ) 
        self.pred_var = torch.nn.Linear(64, state_dim ) 
        # learnable lagrange multiplier
        self.lambda_csd = torch.nn.Parameter(torch.tensor(lambda_init)) #lambda_csd in the paper
        self.eps = epsilon
    ...
    def predict(self, s):
        x = s/self.scale_l
        x=torch.nn.functional.relu(self.pred_l1(x))
        x=torch.nn.functional.relu(self.pred_l2(x))
        s_next=self.pred_l3(x)
        log_var_s_next=self.pred_var(x)
        return s_next, log_var_s_next
    
    def d_csd(self, s, s_next, reduce=True):
        mu_s_next, logvar_s_next = self.predict(s)
        dist = torch.distributions.Normal(mu_s_next, torch.exp(logvar_s_next))
        log_prob = dist.log_prob(s_next/self.scale_l).sum(dim=-1)
        return - log_prob
    
    def mlh_loss(self, s, s_next, d):
        mu_s_next, logvar_s_next = self.predict(s)
        dist = torch.distributions.Normal(mu_s_next, torch.exp(logvar_s_next))
        log_prob = dist.log_prob(s_next/self.scale_l)
        return -log_prob.mean()

    def lambda_loss(self, s, s_next, z, d):
        phi_s = self(s)
        phi_s_next = self(s_next)
        loss_lambda = (self.lambda_csd*torch.min(self.eps*torch.ones_like(d), self.d_csd(s,s_next) - ((phi_s_next - phi_s)* z).sum(dim=1) ).detach()).mean()
        return loss_lambda
...
"""
    ...
    discriminator_CSD = Discriminator_CSD(state_dim = np.array(envs.single_observation_space.shape).prod(),
                                                z_dim = args.nb_skill,
                                                env_name = args.env_id,
                                                device = device,
                                                lambda_init = args.lambda_CSD_init,
                                                epsilon = args.epsilon_CSD,
                                                scale_l = 1.0,
                                                lip_cte = args.lip_cte_CSD).to(device)
    qf1_target.load_state_dict(qf1.state_dict())
    qf2_target.load_state_dict(qf2.state_dict())
    q_optimizer = optim.Adam(list(qf1.parameters()) + list(qf2.parameters()), lr=args.q_lr)
    actor_optimizer = optim.Adam(list(actor.parameters()), lr=args.policy_lr)
    discriminator_CSD_optimizer = optim.Adam(list(discriminator_CSD.parameters()), lr=args.lr_discriminator_CSD)
    lambda_CSD_optimizer = optim.Adam([discriminator_CSD.lambda_csd], lr=args.lr_lambda_CSD)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if (global_step)%1_000_000==0 :
            envs.call("set_max_steps", CSD_max)
            CSD_max  = min(CSD_max + 200, config[args.env_id]['kwargs']['max_episode_steps'])
        ...
        if global_step*args.num_envs > args.learning_starts and  global_step % int((args.CSD_max_step * args.episode_per_epoch)/args.num_envs) == 0:
            for _ in range(args.CSD_discriminator_epochs):
                # CSD training
                batch_inds = np.random.randint(0 ,rb.pos if not rb.full else rb.buffer_size, int(args.CSD_batch_size))                    
                batch_inds_env = np.random.randint(0, args.num_envs, args.CSD_batch_size)
                ...
                loss_mlh = discriminator_CSD.mlh_loss(batch_obs, batch_next_obs, batch_dones)
                # embedding loss
                loss_CSD, inner_product_loss = discriminator_CSD.lipshitz_loss(batch_obs, batch_next_obs, batch_z, batch_dones) 
                total_loss_CSD = loss_CSD + loss_mlh
                discriminator_CSD_optimizer.zero_grad()
                total_loss_CSD.backward()
                discriminator_CSD_optimizer.step()
                # lambda loss
                lambda_loss = discriminator_CSD.lambda_loss(batch_obs, batch_next_obs, batch_z, batch_dones)
                lambda_CSD_optimizer.zero_grad()
                lambda_loss.backward()
                lambda_CSD_optimizer.step()

                wandb.log({
                    # losss
                    "losses_CSD/discriminator_loss": loss_CSD.item(), 
                    "losses_CSD/inner_product_loss": inner_product_loss.item(),
                    }, step = global_step) if args.track else None
        ...
        if global_step*args.num_envs > args.learning_starts and global_step % args.learning_frequency == 0:
            ...
            with torch.no_grad():
                ...
                # rewards
                CSD_reward = ((discriminator_CSD(b_next_observations) - discriminator_CSD(b_observations)) * b_z).sum(dim = -1) 
                # print('CSD reward shape : ', CSD_reward.shape)
                intrinsic_reward = CSD_reward 
            ...
            if global_step % 100 == 0:
                ...
                "metrics/CSD_reward_mean": CSD_reward.mean().item(), 
                "metrics/CSD_reward_max": CSD_reward.max().item(), 
                "metrics/CSD_reward_min": CSD_reward.min().item(),
